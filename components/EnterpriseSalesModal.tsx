...
import { X, Mail, Building2, User, Loader2, CheckCircle2 } from 'lucide-react';
...
const EnterpriseSalesModal: React.FC<EnterpriseSalesModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // In a more robust setup, you might use a CORS-enabled backend proxy.
      console.log('Form submission initiated. Assuming success due to no-cors mode.');
      setStatus('success');
      setFormData({ // Clear form
        name: '', company: '', email: '', phone: '', estimatedHeadshots: '', message: '',
      });
    } catch (error: any) {
      ...
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-8">
          <div className="text-center space-y-2 mb-8">
            <div className="mx-auto w-12 h-12 bg-purple-900/30 rounded-xl flex items-center justify-center mb-4">
              <Building2 className="w-6 h-6 text-purple-400" />
            </div>
            <h2 className="text-2xl font-bold text-white">Enterprise Sales Inquiry</h2>
            <p className="text-slate-400">Tell us about your team's needs, and we'll get back to you.</p>
          </div>
          ...
          {status === 'success' ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-slate-400 max-w-xs">We've received your message and will contact you shortly.</p>
              <button 
                onClick={onClose} 
                className="mt-8 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold transition-colors"
              >
                Close
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label htmlFor="estimatedHeadshots" className="text-sm font-medium text-slate-300">Estimated Headshots Needed</label>
                <select
                  id="estimatedHeadshots"
                  name="estimatedHeadshots"
                  value={formData.estimatedHeadshots}
                  onChange={handleChange}
                  required
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent appearance-none"
                >
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-y"
                ></textarea>
              </div>
